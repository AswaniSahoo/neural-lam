...
| Method | Lines | Purpose |
|---|---|---|
| `__init__` | 19-84 | Sets up edge_index, edge_mlp (3·d_h → d_h), aggr_mlp (2·d_h → d_h). Supports chunked MLPs. |
...
| `DEFINED_METRICS` | 230-237 | Registry dict mapping names to functions. |
...
| Function | Lines | Purpose |
|---|---|---|
| `plot_error_map` | 13-66 | Heatmap of `(pred_steps × d_f)` errors. Normalizes per variable, uses `OrRd` colormap. |
| `plot_prediction` | 70-126 | Side-by-side plot of ground truth vs. prediction on a spatial map with coastlines. |
| `plot_spatial_error` | 130-185 | Spatial error map on the grid with coastlines and colorbar. |
