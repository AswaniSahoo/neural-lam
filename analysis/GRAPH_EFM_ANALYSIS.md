...
5. [predict_step vs unroll_prediction_vi (VI ≠ Prior Sampling)](#5-predict_step-vs-unroll_prediction_vi)
...
> ⚠️ **Note:** `plot_examples` has a **different signature** in GraphEFM (`self, batch, n_examples, prediction=None`) vs ARModel (`self, batch, n_examples, split, prediction=None`) — GraphEFM is missing the `split` parameter. This is an API incompatibility to resolve during refactoring.

> ⚠️ **Note:** `on_validation_epoch_end` logs spsk_ratio **BEFORE** calling `super()` (because `super()` clears metric lists), while `on_test_epoch_end` calls `super()` **FIRST** then logs. This ordering asymmetry is a potential source of bugs.
...
ForecasterModule(pl.LightningModule)
    ...
    ├── training_step, validation_step, test_step (delegates to Forecaster)
    ├── MetricsCallback (pluggable)
    └── PlottingCallback (pluggable)

Forecaster(ABC, torch.nn.Module)
    ├── unroll_prediction (AR rollout or direct forecast)
    └── common_step (unpack batch + call unroll)

StepPredictor(ABC, torch.nn.Module)
    ├── predict_step (deterministic)
    └── predict_step_vi + sample_trajectories (probabilistic)
```

> **Note:** This matches Joel's #49 design: lifecycle hooks (`training_step`, etc.) stay in the `ForecasterModule` (Lightning wrapper), while the forecast logic lives in `Forecaster`/`StepPredictor` (pure `torch.nn.Module`).
